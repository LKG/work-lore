		...
		$.httpUtil.curl({
					...
					success : function(data) {
						if (data.success) {
							...
							$select.empty();
							$select.append($("<option>").text("无").val("0").attr("data-itemCode",$mainForm.find("#itemCode").val()));
							$.each(data.result.content, function(index, item) {
								var $option = $("<option>").text(item.itemName+"("+item.itemCode+")").val(item.itemId);
                                $option.attr("data-itemCode",item.itemCode+"_");
								$select.append($option);
							});
                            $select.on("change",function () {
                                $mainForm.find("#itemCode").val($(this).find("option:selected").attr("data-itemCode"));
                            });
							// 加载分页组件
							laypage({
